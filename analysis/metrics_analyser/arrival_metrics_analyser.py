...
def analyse_arrival(parquet_path: Path, run_id: str) -> None:
    ...
    aggregates per time slot to produce:
      ...
      - path_deviation_km_*  : percentile distribution of route deviation in km
      - delta_arrival_*      : percentile distribution of arrival time delta in seconds
    """
    ...
    snapshot_df = df.with_columns([
        (pl.col("PathDeviation") / METRES_PER_KM).alias("path_deviation_km"),

        (pl.col("DeltaArrivalTime")).alias("delta_arrival_seconds"),
        ...
        "path_deviation_km", "delta_arrival_seconds", "missed_deadline",
    ]).sort(["day", "simtime_ms"])
    ...
    agg_df = (
        ...
        .agg(
            ...
            + [pl.col("path_deviation_km").quantile(q).alias(f"path_deviation_km_p{int(q * 100)}")
               for q in percentiles]
            + [pl.col("delta_arrival_seconds").quantile(q).alias(f"delta_arrival_s_p{int(q * 100)}")
               for q in percentiles]
        )
